(ns watchmfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control movement-assembly/casing/
  regulation/testing equipment directly... does NOT authorize or
  execute maintenance/equipment operations... does NOT self-issue a
  chronometer/accuracy certification mark') implemented faithfully.
  The single invariant under test:

    WatchOpsAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Watch & Clock Plant
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :automatic-watch}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :automatic-watch (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-movement-line :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest certification-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-certification? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the chronometer/accuracy certification authority"
    (let [[db actor] (fresh)
...
(deftest invalid-accuracy-test-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:accuracy-test-seconds-per-day 999999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-accuracy-test-seconds-per-day} (-> (store/ledger db) last :basis)))
    (is (not= 999999.0 (:accuracy-test-seconds-per-day (store/batch db "batch-001"))) "fabricated accuracy-test reading never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "assembly-001" :severity :moderate
                                            :description "battery-cell leakage anomaly"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
