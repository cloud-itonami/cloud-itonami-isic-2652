(ns watchmfg.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `watchmfg.store` ns docstring for why a second (Datomic-backed)
...
(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:product-type :quartz-watch}})
    (is (= :quartz-watch (:product-type (store/batch s "batch-001"))))
