(ns watchmfg.operation-test
  "Smoke tests for the compiled WatchOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `watchmfg.governor-contract-test`; the Store contract in
  `watchmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "WatchOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "assembly-001" :severity :moderate :description "battery-cell leakage suspected"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
