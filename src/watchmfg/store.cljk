(ns watchmfg.store
  "SSoT for the watch/clock plant-operations coordination actor, behind
  ...
  `elecequipmfg.store`, `cloud-itonami-isic-2211`'s own
  `tyremfg.store`), this build ships a single `MemStore` backend only
  ...
  self-contained (no external watch/clock-manufacturing capability
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/accuracy-test-seconds-per-day/
    ...
    - `equipment`         -- a movement-assembly/casing/regulation/
                             testing unit's own record. `:verified?`/
    ...
    - `maintenance`       -- a scheduled movement-assembly/casing/
                             regulation/testing maintenance-window
    ...
    - `shipments`         -- a proposed outbound watch/clock shipment
                             DRAFT (`watchmfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (watchmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (watchmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :automatic-watch
                :model "AW-100" :accuracy-test-seconds-per-day 5.0
                :quantity-units 500.0 :defect-rate-percent 0.8
                :verified? true :registered? true
                :shipped-units 100.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :quartz-watch
                :model "QZ-200" :accuracy-test-seconds-per-day 0.3
                :quantity-units 200.0 :defect-rate-percent 0.5
                :verified? true :registered? true
                :shipped-units 180.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :wall-clock
                :model "WC-15" :accuracy-test-seconds-per-day 15.0
...
(defn- sample-equipment []
  {"assembly-001" {:id "assembly-001" :kind :movement-assembly-station
                    :verified? true :registered? true
                    :last-maintenance-date "2026-05-01"}
   "test-bench-002" {:id "test-bench-002" :kind :accuracy-test-bench
                      :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `watchmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `watchmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  movement-assembly-station unit (schedulable for maintenance), one
  UNVERIFIED/unregistered accuracy-test-bench unit (blocks any
