(ns watchmfg.governor
  "Watch & Clock Plant Operations Governor -- the independent
  compliance layer that earns the WatchOpsAdvisor the right to commit.
  ...
  movement-assembly/casing/regulation/testing equipment, whether a
  proposal secretly tries to self-issue a chronometer/accuracy
  ...
  coordination proposal and becomes direct movement-assembly/testing-
  ...
  *reject* a proposal and fall back to HOLD.
  ...
  `:watch-clock-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       'direct movement-assembly/
                                       testing-equipment control' scope
    ...
    4. Equipment-actuate blocked   -- for `:schedule-maintenance`, does
                                       ...
                                       regulation/testing equipment is
    ...
    5. Certification-authority
       blocked                     -- ANY proposal (any op) whose own
                                       ...
                                       certification mark (e.g. COSC
                                       under ISO 3159) -- an authority
   ...
   13. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [watchmfg.registry :as registry]
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct movement-
  assembly/casing/regulation/testing-equipment-control effect."
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct movement-assembly/casing/regulation/testing-
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") はムーブメント組立・ケーシング・調整・試験設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- equipment-actuate-blocked-violations
  ...
  to directly actuate movement-assembly/casing/regulation/testing
  ...
  (when (and (= op :schedule-maintenance)
    ...
    [{:rule :equipment-actuate-blocked
      :detail "ムーブメント組立・ケーシング・調整・試験設備の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- certification-authority-blocked-violations
  ...
  to self-issue a chronometer/accuracy certification mark (e.g. COSC
  under ISO 3159) -- an authority exclusively reserved to the
  ...
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-certification? payload))
      [{:rule :certification-authority-blocked
        :detail "クロノメーター/精度認証(COSC等)の自己発行提案は恒久的に禁止 -- 認証機関の専権事項"}])))
...
(defn- invalid-accuracy-test-violations
  "For `:log-production-batch`, if the patch declares an
  `:accuracy-test-seconds-per-day` that is not a physically plausible
  rate-test reading, reject rather than let fabricated/sensor-error
  data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [spd (:accuracy-test-seconds-per-day (:value proposal))]
      (when (and (some? spd) (not (registry/accuracy-test-seconds-per-day-valid? spd)))
        [{:rule :invalid-accuracy-test-seconds-per-day
          :detail (str spd "秒/日 は物理的に妥当な精度試験結果の範囲外")}]))))
...
(defn check
  "Censors a WatchOpsAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-accuracy-test-violations request proposal)
