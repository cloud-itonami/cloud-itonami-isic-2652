(ns watchmfg.advisor
  "WatchOpsAdvisor -- the *contained intelligence node* for the
  watch/clock plant-operations coordination actor.

  It normalizes production-batch patches (product-type/accuracy-test-
  seconds-per-day/quantity/defect-rate), drafts a movement-assembly/
  casing/regulation/testing-equipment maintenance scheduling proposal
  against a piece of equipment, drafts a safety-concern flag, and
  drafts an outbound watch/clock shipment coordination proposal
  ...
  cited), never a committed record and NEVER a real movement-assembly/
  testing-equipment actuation, freight dispatch, or chronometer/
  accuracy certification -- see README `What this actor does NOT do`.
  Every output is censored downstream by `watchmfg.governor` before
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct movement-
                                 ; assembly/casing/regulation/testing-
  ...
  `watchmfg.governor` HARD-holds any request that doesn't, so a
...
(defn- log-production-batch
  ...
  accuracy-test-seconds-per-day, quantity, defect-rate or verification
...
(defn- schedule-maintenance
  "Draft a movement-assembly/casing/regulation/testing-equipment
  ...
  registered?) in its rationale, but `watchmfg.governor` NEVER trusts
...
(defn- flag-safety-concern
  "Draft a materials-safety (battery/mercury-cell hazard)/precision-
  defect concern. ALWAYS `:stake :coordination/safety-concern` -- a
  safety concern is NEVER a proposal the advisor may quietly downgrade
  to low-stakes, and it is never gated on the referenced equipment/
  batch being verified (a concern can be raised about ANY equipment or
  ...
  technicality). See `watchmfg.phase`: no phase ever adds this op to a
  phase's `:auto` set; `watchmfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound watch/clock shipment coordination proposal
  against a production batch. The advisor passes through the caller's
  own claimed unit quantity -- it does NOT invent one, and
  `watchmfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたは時計(腕時計・置時計・掛時計)製造工場"
       ...
       "ムーブメント組立・ケーシング・調整・試験等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "クロノメーター/精度認証(COSC等)を自己発行する提案をしてはいけません。"
       "出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `watchmfg.governor`
