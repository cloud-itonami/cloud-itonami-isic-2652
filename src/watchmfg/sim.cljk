(ns watchmfg.sim
  ...
  production quantity, a proposal that tries to ACTUATE movement-
  assembly/casing/regulation/testing equipment directly (permanently
  blocked, no override), a proposal that tries to self-issue a
  chronometer/accuracy certification mark (permanently blocked, no
  override), a double-schedule of the same maintenance window, a
  production-batch patch with a fabricated product type, a
  production-batch patch with an implausible accuracy-test reading,
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on assembly-001 (verified, registered, movement-assembly station -- escalates, approve) ==")
    ...
    (println "== schedule-maintenance mnt-2 on test-bench-002 (UNVERIFIED/unregistered accuracy test bench -> HARD hold) ==")
    ...
    (println "== log-production-batch batch-001 with an implausible accuracy-test reading -> HARD hold ==")
    ...
    (println "== log-production-batch batch-001 attempting to self-issue a chronometer/accuracy certification -> HARD hold, PERMANENT ==")
