(ns watchmfg.operation
  "WatchOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (WatchOpsAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Watch & Clock
...
(defn build
  "Compiles a WatchOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `watchmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; WatchOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Watch & Clock Plant Operations Governor -- independent censor.
      (g/add-node :govern
