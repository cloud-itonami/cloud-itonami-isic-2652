(ns watchmfg.registry
  "Pure-function domain logic for the watch/clock plant-operations
  ...
  pre-existing `kotoba-lang/watchmfg`-style capability library to wrap
  ...
  `cloud-itonami-isic-2710`, and `tyremfg.registry` from
  ...
  coordinated shipment), not the act of actuating movement-assembly/
  casing/regulation/testing equipment or dispatching a real freight
  carrier, and never the act of issuing a chronometer/accuracy
  certification mark (this actor NEVER does any of those -- see README
  `What this actor does NOT do`).

  SCOPE: ISIC 2652 covers manufacture of watches and clocks -- movement
  assembly (mechanical and quartz), casing, regulation, and testing
  (including accuracy/rate testing against a reference timebase) lines
  producing finished watches, clocks, and watch/clock movements. This
  ...
  flagging, shipment coordination) -- it never touches the movement-
  assembly/casing/regulation/testing equipment directly, and it never
  stands in for the certification body that issues chronometer/
  accuracy certification marks (e.g. COSC under ISO 3159).")
...
(def valid-product-types
  ...
  through."
  #{:mechanical-watch :automatic-watch :quartz-watch :chronograph-watch
    :wall-clock :mantel-clock :movement})

(def accuracy-test-seconds-per-day-min
  "Physical floor for a batch's own accuracy/rate-test reading, in
  seconds of deviation per day. A mechanical movement can legitimately
  run slow (negative) as well as fast (positive), so unlike a voltage
  or defect-rate reading this floor is NOT zero -- see
  `accuracy-test-seconds-per-day-max` for the paired ceiling and its
  rationale."
  -60.0)

(def accuracy-test-seconds-per-day-max
  "Physical ceiling for a batch's own accuracy/rate-test reading, in
  seconds of deviation per day. Grounded in general horological
  regulation practice: even an unregulated/economy-grade mechanical
  movement fresh off a production line is routinely brought within
  about a minute per day before shipment, and COSC chronometer
  certification (ISO 3159) requires a far tighter -4/+6 s/day band well
  inside this outer bound. A reading beyond +/-60 s/day is implausible
  test/sensor data -- a stopped movement, a data-entry error, or a unit
  that never should have left the regulation station -- not a real
  routine accuracy-test result on any standard watch/clock movement
  grade."
  60.0)

(def defect-rate-min-percent
  "Physical floor for a batch's own movement-assembly/casing/testing
  defect-rate reading (zero defects is the best possible outcome,
  never negative)."
  0.0)

(def defect-rate-max-percent
  "Physical ceiling for a batch's own movement-assembly/casing/testing
  defect-rate reading -- a batch cannot reject more than 100% of its
  own output. A reading above this is implausible sensor/QC data, not
  a real batch."
  100.0)
...
(defn batch-verified?
  ...
  (i.e. its product-type/accuracy-test/quantity/defect-rate claims
...
(defn accuracy-test-seconds-per-day-valid?
  "Is `seconds-per-day` a physically plausible accuracy/rate-test
  reading, in seconds of deviation per day? Rejects nil, non-numbers,
  and values beyond the +/- 60 s/day physical plausibility band -- a
  fabricated or sensor-error reading, never let through as a real test
  result."
  [seconds-per-day]
  (and (number? seconds-per-day)
       (>= (double seconds-per-day) accuracy-test-seconds-per-day-min)
       (<= (double seconds-per-day) accuracy-test-seconds-per-day-max)))

(defn defect-rate-valid?
  "Is `percent` a physically plausible batch movement-assembly/casing/
  testing defect-rate reading? Rejects nil, non-numbers, negative
...
(defn- unsigned-certificate
  ...
  actor's. And NEVER a chronometer/accuracy certification mark -- this
...
(defn register-maintenance
  ...
  movement-assembly/casing/regulation/testing-equipment maintenance
  window against a verified, registered piece of equipment. Pure
  function -- does not actuate the movement-assembly/testing equipment
  or execute any maintenance; it builds the RECORD a plant coordinator
  would keep. `watchmfg.governor` independently re-verifies the
  equipment's own verified/registered ground truth, and permanently
  blocks any attempt to directly actuate the equipment (see README
...
(defn register-shipment
  ...
  outbound watch/clock shipment against a verified, registered
  ...
  `watchmfg.governor` independently re-verifies the shipment's own
